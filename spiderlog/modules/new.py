...
from spiderlog.modules import verification_values
import shutil
import subprocess
import json
...
def main():
    ...
    if os.path.exists(PROJECT_NAME):
        ...
        if verification_values.yes_or_no(overwrite_and_continue):
    ...
    while not stylesheet_type.upper() in ["CSS", "SCSS", "SASS"]:
        message.error(f"Select Stylesheets type value is invalid.")
        stylesheet_type = input(
            "Which stylesheet types do you use, CSS or SCSS or SASS?:")
    using_git: str = input(
        "Would you like to use version control system (git):")
    if verification_values.yes_or_no(using_git):
        try:
    ...
    message.success("Finished making your new project.")
